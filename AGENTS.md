...
- **`declare -f` probes into `bin/` are a shared shim, never a per-file copy.** A cleanup lib reaching for `safe_clean_guarded` or `defer_cleanup_family` is asking whether `bin/clean.sh` is loaded. It always is in production (`bin/clean.sh` is the only entry point that sources `lib/clean/*`), and never is in a standalone Bats case, so every such branch is a second degraded copy of a delete decision that only tests execute. Three byte-identical copies of the defer probe grew across `dev.sh` / `user.sh` / `app_caches.sh` before it became `mole_defer_cleanup_family` in `lib/core/base.sh`; the ten remaining `safe_clean_guarded` fallbacks are audited and capped. New cleanup code calls `safe_clean_guarded` directly and lets the test supply it. Locked by `cleanup libs share one engine-absent shim instead of forking their own` and `engine-absent cleanup fallbacks stay at their audited count` in `tests/clean_core.bats`; lowering the cap after a removal is expected, raising it needs a stated reason. Rejected: hoisting `_safe_clean_impl` and its five `bin/clean.sh` helpers into `lib/core/` to delete the fallbacks outright. That would bypass the `safe_clean` stub 235 test points rely on, turning stubbed assertions into real deletions under a temp `HOME`.
